...
def init_db():
    with get_conn() as conn:
        ...
        # Таблица для регламентов/штрафов
        cur.execute("""
...
# --- ФУНКЦИЯ АВТО-НАПОЛНЕНИЯ (СЕЙДИНГ) ---
def check_and_seed_data():
    """Проверяет, есть ли данные в таблице regulations. Если нет - заполняет."""
    with get_conn() as conn:
        cur = conn.cursor()
        ...
        if count == 0:
            print("⚠️ База пуста. Начинаю автоматическое наполнение регламентов...")
            ...
            # Очищаем таблицу (на случай частичной записи) и вставляем данные
            ...
            print(f"✅ База уже содержит {count} записей. Пропускаем наполнение.")


# --- ОСТАЛЬНЫЕ ФУНКЦИИ ДЛЯ ИНСТРУКЦИЙ (без изменений) ---
